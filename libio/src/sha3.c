...
sha3_Update(void *priv, void const *bufIn, size_t len)
{
  ...
  if (len < old_tail)
  {
    ...
    SHA3_TRACE("because %d<%d, store it and return", (unsigned int)len, (unsigned int)old_tail);
  ...
  if (old_tail)
  {
    ...
    SHA3_TRACE("completing one word with %d bytes", (unsigned int)old_tail);
  ...
  SHA3_TRACE("have %d full words to process", (unsigned int)words);
  ...
  SHA3_TRACE("have %d bytes left to process, save them", (unsigned int)tail);
